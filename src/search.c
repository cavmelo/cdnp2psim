...
// random walk
static void* runRandomWalkSearch(TSearch *search, void *vPeer, void *object, unsigned int clientId ){
	TPeer *peer = vPeer;
	...
	do{
		...
		if (!peer->canStream(peer,object, clientId)){
...
static void* runFloadingSearch(TSearch *search, void* vPeer, void* object, unsigned int clientId){
	TPeer *peer = vPeer;
	TDataSearch *data = search->data;
